...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const systemMessage = { role: 'system', content: 'You are called 𝙱.𝙼.𝙱-𝚇𝙼𝙳. Developed by 𝙱.𝙼.𝙱-𝚇𝙼𝙳. You respond to user commands. Only mention developer name if someone asks.' };
...
zk.ev.on("messages.upsert", async (m) => {
    ...
    if (messageContent.slice(1).toLowerCase() === "vcf") {
        ...
        const baseName = "𝙱.𝙼.𝙱-𝚇𝙼𝙳 BOT FAMILY";
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            console.log("\t🌍𝙱.𝙼.𝙱-𝚇𝙼𝙳-BOT ONLINE🌍");
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*𝙱.𝙼.𝙱-𝚇𝙼𝙳 WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Pkdriller is connecting...");
            }
                ...
                console.log("------------------/-----");
                console.log("𝙱.𝙼.𝙱-𝚇𝙼𝙳 is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading 𝙱.𝙼.𝙱-𝚇𝙼𝙳 Commands ...\n");
